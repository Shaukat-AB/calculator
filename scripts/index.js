...
keys.addEventListener ( 'click', e => {
   if ( e.target.matches('button') ) {
      let dataValue = e.target.dataset.value, dataFunction = e.target.dataset.function;
...
const withDataFunctions = (dataFunction) =>{
   switch( dataFunction ){
      ...
      case "equal": equalTo(expression.value); break;
      case "del": del() ; break;
   }
}
...
const mexp = new Mexp()
mexp.addToken([{token: '%', show: '%', type: 7, precedence: 11, value: (x) => x/100 }]);
mexp.addToken([{token: 'sqrt', show: 'sqrt', type: 0, precedence: 11, value: Math.sqrt }]);
mexp.addToken([{token: 'abs', show: 'abs', type: 0, precedence: 11, value: Math.abs }]);

const validateExp = ( value, exp ) => {
   let tokens = [];
   mexp.tokens.forEach( e => tokens.push(e.token) );
   let lastValue = exp!='' && exp[exp.length - 1] || '';
   if( value=='' || tokens.indexOf(value) < 0 || ' n,'.includes(value) ) return false;
   else if( !isNumber(lastValue) && !isNumber(value) && '*/+'.includes(value) && !'%pi!e'.includes(lastValue) ) return false;
...
const equalTo = ( exp ) => {
   ...
   history.push(ent);
   history.length && history.forEach( (e, i) => ( e.expression == ent.result ) && ( history = history.slice(0, i-1).concat( history.slice( i+1, history.length ) ) ) );
...
let toggleVar = { "dark": false, "unhide": false, "histOpen": false , __proto__:null };
...
const darkMode = () =>{
   ...
   toggler( "dark", ()=> body.classList.add('dark-mode'), ()=> body.classList.remove('dark-mode'), toggleVar);
}

const toggleHide = () => {
   let container = keys.querySelector('.hidden');
   toggler( "unhide", () => container.style.display = 'grid', ()=> container.style.display = 'none', toggleVar);
}

const toggleDeg = (btn) =>{
   toggler("isDegree", ()=> btn.textContent = 'deg', ()=> btn.textContent = 'rad', mexp.math );
}

toggleHistory = () => {
   let container = document.querySelector('.history');
   toggler("histOpen", ()=> container.style.display = 'flex', ()=> container.style.display = 'none', toggleVar );
}
...
expression.addEventListener('input', e => ( e.key !='Tab' ) && e.preventDefault() );
